...
function getShowPayWay() {
	return ['pc'] // 'zhifubao', 'weixin',  'weixinWeb', 'zhifubaoWeb', 'kuaijie',
}
...
function getWXInviteTitleWL () {
	return '%s 在易創錢赚的太嗨，送美金给你，注册后立即到账！';
}

// 微信邀请好友描述
// 0526更新
function getWXInviteDesWL () {
	return '易創錢，注册就送美金助你投资全球，盈利分分钟可提现！';
}
...
function getAboutUSHTMLWL () {
	return '<div class="bd">\
			<p>目前，易創錢已经支持全球20个交易所的近8000个品种，所有品种通过单一交易账户便可访问。</p><br/>\
			<p>易創錢团队在金融领域拥有多年从业经验并掌握大量行业资源，在创立之初就确立了“为大众用户提供最优惠、性价比最高的交易服务”的宗旨，并一直坚定地奉行这一宗旨，致力于在股票、期货、外汇、贵金属及其他各类市场中寻求最优秀的流动性。</p><br/>\
			<p>易創錢团队在计算机网络技术方面亦颇有建树，在交易系统的构建过程中充分考虑了稳定性、时效性、易用性等多方面因素，力求为投资者提供极致的交易体验。</p><br/>\
			<p>易創錢的使命是让全球每一个人，无论处于何等收入水平，都能够以简易、安全、快捷的方式参与投资全球金融市场。</p><br/>\
			</div>';
}
...
function getMobilePayTextWL() {
	return '易創錢通过多家三方支付公司合作，确保您的账户与资金安全。'  ;
}
...
function getNewGuideQRCode () {
	return '../img/wl/eccoinage/eccoinage_gw_qrcode.jpg';
}
...
function getRiskMsg() {
	return '<p>尊敬的易創錢客户:</p>\
	<p class="first">易創錢交易平台的电子交易业务是一种潜在收益和潜在风险较高的投资业务,对投资者的风险承受能力、理解风险程度、风险控制能力以及投资经验有较高的要求。</p>\
