...
class Register extends PageBase {
  ...
  _submit(tel) {
    var dialog = this.dialog,
      self = this;
    var submitEl = $('.submit');
    var invCodeEl = $('.wrapper.second .inv_code');
    self.referCode = self.referCode ? self.referCode : invCodeEl.val();
  ...
  _goRegister() {
    ...
    if (this.isWeixin()) {
      ...
      $('.optionUrl').attr('href',url);

      var wx = '<a class="aa" href="https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=' + getWXIDWL() +'&scene=124#wechat_redirect">点击关注微信公众号 享免费资讯服务</a>'
    ...
    // 直接跳转版本 
      ...
      var url = window.location.origin + wl + '/s/option.html';
      $('.optionUrl').attr('href',url);
    }

    $('.dialog-mask').eq(0).show();
    $('.success-wrapper',cont).show();
  }
  ...
  _change(e) {
    ...
    } else if ( curEl.hasClass('inv_code') ) {
      error = validator.validateField({
        name: 'inv_code',
        rules: 'required',
        value: value
  ...
  _initAttrs() {
    ...
    this.hasReferCode(this.referCode);
