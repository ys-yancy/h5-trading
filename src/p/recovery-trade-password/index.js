...
function RecoveryTradePassword() {
  RecoveryTradePassword.superclass.constructor.apply(this, arguments);
  var self = this;
...
Base.extend(RecoveryTradePassword, PageBase, {
  init: function() {
    this._initAttrs();
    this._initValidate();
    this._bind();
  ...
  _submit: function(e) {
    ...
    var url = getSimulatePlate() ? '/v1/user/real/tradepassword/set/' : '/v1/user/real/tradepassword/setnew/';
    this.ajax({
      url: url,
      type: 'post',
      data: {
  ...
  _initAttrs: function() {
    ...
    $('.J_TradeHtmlLink').attr('href', '././recovery-password.html?src=' + src);
  },
