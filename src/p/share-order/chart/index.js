...
var ChartArea = require('../../../common/chart/share');
...
export default class Chart extends Base {
  ...
  _getCandle(type, refresh, chartType) {
    var self = this;
    ...
    this.ajax({
      ...
      for (var i = data.price.length - 1, count = 0; i > 0; i--) {
        ...
        if (chartType === 'area') {
          ...
          // console.log((item.open + item.close)/2, this.unit)
        } else {
          list.push([
            Util.getTime(item.beijing_time),
            item.open,
      ...
      // this.chart.addPlotLine(this.openPrice);

      // if (this.stopLoss) {
      //   this.chart.addPlotLine(this.stopLoss, 'stopLoss');
      // }
      // if (this.takeProfit) {
      //   this.chart.addPlotLine(this.takeProfit, 'takeProfit');
      // }
    });
  }
